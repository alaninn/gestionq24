...
function CuentasCorrientes() {
  ...
  // Agregar deuda a mano (préstamo, artículo fuera de stock, etc.)
  const [mostrarModalDeuda, setMostrarModalDeuda] = useState(false);
  const [formDeuda, setFormDeuda] = useState({ monto: '', nota: '' });
...
const registrarPago = async (e) => {
  ...
  const registrarDeuda = async (e) => {
    e.preventDefault();
    setError('');
    if (!(parseFloat(formDeuda.monto) > 0)) { setError('El monto debe ser mayor a 0'); return; }
    try {
      await api.post(`/api/clientes/${clienteSeleccionado.id}/deuda`, { monto: formDeuda.monto, nota: formDeuda.nota });
      setExito('Deuda cargada correctamente');
      setMostrarModalDeuda(false);
      setFormDeuda({ monto: '', nota: '' });
      const res = await api.get(`/api/clientes/${clienteSeleccionado.id}`);
      setDetalleCliente(res.data);
      setClienteSeleccionado(res.data);
      cargarClientes();
      ...
      setError(err.response?.data?.error || 'Error al cargar la deuda');
    }
  };
  ...
  return (
    <div className="space-y-4">
                ...
                {cargando ? (
                  ...
                  <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          <button onClick={(e) => { e.stopPropagation(); verDetalle(cliente); setFormDeuda({ monto: '', nota: '' }); setMostrarModalDeuda(true); }}
                            title="Agregar deuda a mano (préstamo, artículo fuera de stock...)"
        ...
        {detalleCliente && (
          <div className="w-80 bg-white rounded-xl shadow overflow-hidden flex-shrink-0">
            ...
            <div className="p-3 border-b flex gap-2">
              ...
              <button onClick={() => { setFormDeuda({ monto: '', nota: '' }); setMostrarModalDeuda(true); }}
                title="Agregar deuda a mano"
            ...
            <div className="overflow-y-auto max-h-96 p-3 space-y-2">
              ...
              {/* Pagos y deudas cargadas a mano (pagos_deuda) */}
              {detalleCliente.pagos?.map(pago => {
                const esDeuda = pago.tipo === 'deuda';
                return (
                  <div key={`pago-${pago.id}`} className={`rounded-lg p-2.5 border ${esDeuda ? 'bg-amber-50 border-amber-200' : 'bg-green-50 border-green-200'}`}>
                    <div className="flex justify-between items-start">
                      <div>
                        <p className={`text-xs font-medium ${esDeuda ? 'text-amber-700' : 'text-green-700'}`}>{esDeuda ? '➕ Deuda agregada' : '✅ Pago'}</p>
                        {!esDeuda && <p className="text-xs text-gray-500">{pago.metodo_pago}</p>}
                        {pago.nota && <p className="text-xs text-gray-400">{pago.nota}</p>}
                      </div>
                      <div className="text-right">
                        <p className={`font-bold ${esDeuda ? 'text-amber-600' : 'text-green-600'}`}>{esDeuda ? '+' : ''}{fmt(pago.monto)}</p>
      ...
      {/* Modal AGREGAR DEUDA a mano */}
      {mostrarModalDeuda && clienteSeleccionado && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="p-5 border-b bg-amber-500 text-white rounded-t-xl">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-bold">➕ Agregar deuda</h3>
                  <p className="text-amber-100 text-sm">Cliente: {clienteSeleccionado.nombre}</p>
                </div>
                <button onClick={() => setMostrarModalDeuda(false)} className="text-amber-100 hover:text-white text-2xl">×</button>
              </div>
            </div>
            <form onSubmit={registrarDeuda} className="p-5 space-y-4">
              {error && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-2.5 rounded-lg text-sm">❌ {error}</div>}
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex justify-between">
                <span className="text-gray-600">Deuda actual</span>
                <span className="font-bold text-red-600">{fmt(clienteSeleccionado.saldo_deuda)}</span>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Monto de la deuda *</label>
                <div className="relative">
                  <span className="absolute left-3 top-2.5 text-gray-500">$</span>
                  <input type="number" value={formDeuda.monto}
                    onChange={(e) => setFormDeuda(p => ({ ...p, monto: e.target.value }))}
                    required autoFocus min="0"
                    className="w-full border border-gray-300 rounded-lg pl-7 pr-3 py-2.5 text-lg focus:outline-none focus:ring-2 focus:ring-amber-500"
                    placeholder="0" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Concepto / nota</label>
                <input type="text" value={formDeuda.nota}
                  onChange={(e) => setFormDeuda(p => ({ ...p, nota: e.target.value }))}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-amber-500"
                  placeholder="Ej: préstamo, artículo fuera de stock, fiado a mano..." />
              </div>
              <div className="flex gap-3">
                <button type="button" onClick={() => setMostrarModalDeuda(false)}
                  className="flex-1 py-2.5 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">
                  Cancelar
                </button>
                <button type="submit"
                  className="flex-1 py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-bold transition-colors">
                  ➕ Agregar deuda
                </button>
              </div>
            </form>
