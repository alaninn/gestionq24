...
// -----------------------------------------------
// RUTA: GET /api/productos/:id/codigos
// FUNCIÓN: Traer códigos alternativos de un producto
// -----------------------------------------------
router.get('/', async (req, res) => {
    try {
        const { buscar, categoria, pagina = 1, limite = 50 } = req.query;
        const negocio_id = req.usuario.negocio_id || 1;
        ...
        const resultado = await db.query(`
            ...
            ORDER BY p.nombre ASC
            LIMIT $${contador} OFFSET $${contador + 1}
       `, [...valores, pagina ? parseInt(limite) : 999999, pagina ? offset : 0]);
